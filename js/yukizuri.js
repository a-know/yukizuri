$(function(){
    ...
    if (!window["WebSocket"]) {
        ...
        socket.onmessage = function(e) {
            ...
            var content = msg.Message.replace(/<("[^"]*"|'[^']*'|[^'">])*>/g,'').replace(exp,"<a href='$1' target='_blank' rel='noopener norefferer'>$1</a>").replace(imageExp,"<img src='data:image/$1' />");
            if (msg.Name == loginUserName) {
                messages.prepend(
                    ...
                    .append(
                        $("<div>").attr("class", "chat-box-text").html(content)
                    )
                );
            } else {
                var icon = "";
